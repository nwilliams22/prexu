//! THROWAWAY feasibility spike — beads prexu-60mz.2 / Path C3b.
//!
//! Extends the C2 overlay spike (same one-DComp-tree video+webview pipeline) with
//! HARDWARE DECODE enabled, to settle the biggest unverified Path C risk: does
//! `hwdec=auto-safe` survive the libmpv2 RenderContext + ANGLE EGL share-handle
//! interop? C0/C2 ran `hwdec=no`. Production needs hwdec for codec breadth (the
//! reason for the native player).
//!
//! It plays a REAL encoded H.264 clip (lavfi/testsrc are CPU-decoded and never
//! engage hwdec), logs mpv's ACTUALLY-chosen decoder via `hwdec-current`
//! (anti-fabrication: never claim hwdec without reading it), and verifies the
//! hardware-decoded frames render correctly into the DComp visual and are
//! WGC-captured. Set PREXU_HWDEC=d3d11va|d3d11va-copy|nvdec|no to probe modes.
//!
...
//! libEGL.dll/libGLESv2.dll on PATH, and an ENCODED video file as argv[1]
//! (sample/test_h264.mp4 is generated by RUN.md's ffmpeg line).
...
fn create_plain_window() -> windows::core::Result<HWND> {
    ...
    let hwnd = unsafe {
        CreateWindowExW(
            ...
            w!("Prexu DComp hwdec Spike (Path C3b)"),
            WS_OVERLAPPEDWINDOW,
            100,
            100,
            WIDTH,
...
fn main() {
    ...
    log::info!("[spike] dcomp-hwdec spike (prexu-60mz.2 / Path C3b) starting");
...
fn run(video: Option<PathBuf>) -> Result<(), String> {
    ...
    for frame in 0..180 {
        ...
        if frame == 30 {
            log::info!("[spike:hwdec] hwdec-current @frame30 = {:?}", mpv_bridge::hwdec_current(&mpv));
        }
        std::thread::sleep(std::time::Duration::from_millis(16));
    }
    log::info!("[spike] render loop done");

    // C3b deliverable: what decoder did mpv ACTUALLY use through the render path?
    let hwdec_now = mpv_bridge::hwdec_current(&mpv);
    let hw_active = !matches!(hwdec_now.as_str(), "no" | "" | "<unknown>");
    log::info!("[spike:hwdec] FINAL hwdec-current = {:?} (hardware_active={})", hwdec_now, hw_active);
    ...
    let out = std::env::current_dir().unwrap_or_default().join("dcomp_hwdec_test.png");
    match capture::capture_window(hwnd, &d3d, &d3d_ctx, &out) {
        Ok(frame) => {
            ...
            // ── C3b verdict: hwdec engaged AND its frames rendered correctly ──
            if hw_active && video_visible {
                println!(
                    "\n=== HWDEC VERDICT: PASS — mpv used hardware decode ({:?}) through the \
                     libmpv2 RenderContext + ANGLE EGL-share path, and the decoded frames \
                     rendered correctly into the DComp visual (video sample {:?} non-black) and \
                     were WGC-captured. hwdec survives Path C rendering. ===\n",
                    hwdec_now, vid
                );
            } else if !hw_active {
                println!(
                    "\n=== HWDEC VERDICT: FAIL — mpv fell back to software (hwdec-current={:?}). \
                     Hardware decode did NOT engage through the render path. Try PREXU_HWDEC=d3d11va; \
                     if still software, the ANGLE default-display device mismatch (C0 risk #3) likely \
                     blocks d3d11va interop — fix = create EGL display over our D3D11 device via \
                     eglGetPlatformDisplayEXT(EGL_ANGLE_d3d11_device). ===\n",
                    hwdec_now
                );
            } else {
                println!(
                    "\n=== HWDEC VERDICT: FAIL — hwdec engaged ({:?}) but frames did NOT render \
                     (video sample {:?} black). Likely the hwdec D3D texture is on a different \
                     device than ANGLE's share path. ===\n",
                    hwdec_now, vid
                );
            }
        }
...
fn verdict(
    ...
    if pass && had_video && video_visible {
        println!(
            ...
            vid, btn, clicked, bar, "dcomp_hwdec_test.png"
        );
...
mod mpv_bridge {
    ...
    pub fn init_mpv() -> Result<Mpv, String> {
        // C3b: hwdec ON. Default auto-safe (mpv picks d3d11va/nvdec on Win);
        // override with PREXU_HWDEC=d3d11va|d3d11va-copy|nvdec|no to probe modes.
        let hwdec = std::env::var("PREXU_HWDEC").unwrap_or_else(|_| "auto-safe".into());
        ...
        log::info!("[spike:mpv] Mpv initialized (vo=libmpv, render API, hwdec={hwdec})");
        Ok(mpv)
    }

    /// mpv's ACTUALLY-chosen decoder. "no"/"" = software; "d3d11va"=zero-copy GPU
    /// interop; "d3d11va-copy"=GPU decode + CPU roundtrip. Anti-fabrication: never
    /// claim hwdec without reading this.
    pub fn hwdec_current(mpv: &Mpv) -> String {
        mpv.get_property("hwdec-current").unwrap_or_else(|_| "<unknown>".into())
    }
