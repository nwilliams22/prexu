...
function parseRatingBadge(
  ...
  return { source: "generic", label: kind === "critic" ? "Critic" : "Audience", display: value.toFixed(1) };
}

/** Build badges from Rating[] array (preferred) or legacy fields */
function buildRatingBadges(
  ...
  if (badges.length === 0) {
    ...
    if (critic) { badges.push(critic); seen.add(critic.source); }
    else if (rating > 0) badges.push({ source: "generic", label: "Critic", display: rating.toFixed(1) });

    const audience = parseRatingBadge(audienceRatingImage ?? "", audienceRating, "audience");
    if (audience && !seen.has(audience.source)) badges.push(audience);
