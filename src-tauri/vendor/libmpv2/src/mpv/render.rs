...
impl RenderContext {
    ...
    /// Prexu vendored addition: identical to [render](Self::render) but passes
    /// `MPV_RENDER_PARAM_BLOCK_FOR_TARGET_TIME = 0`, so the call never parks
    /// the calling thread in mpv's untimed `present_count` cond wait waiting
    /// for the vo thread's `flip_page`. Required when render is driven from a
    /// UI main loop (GTK GLArea render signal): with the default blocking
    /// behavior, a vo-side reconfig race (e.g. `video-margin-ratio-*` changes
    /// mid-playback) deadlocks the vo and the caller against each other and
    /// freezes the entire main loop (prexu-skr2).
    pub fn render_no_block<GLContext>(
        &self,
        fbo: i32,
        width: i32,
        height: i32,
        ...
        let mut raw_params: Vec<libmpv2_sys::mpv_render_param> = Vec::with_capacity(4);
        let mut raw_ptrs: HashMap<*const c_void, DeleterFn> = HashMap::new();
        ...
        raw_params.push(raw_param);
        let raw_param: libmpv2_sys::mpv_render_param =
            RenderParam::<GLContext>::BlockForTargetTime(false).into();
        raw_ptrs.insert(raw_param.data, free_void_data::<i32>);
        raw_params.push(raw_param);
        // the raw array must end with type = 0
        raw_params.push(libmpv2_sys::mpv_render_param {
